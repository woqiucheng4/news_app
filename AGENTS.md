...
Flutter & Dart agent skills installed at `.agents/skills/` (from [flutter/skills](https://github.com/flutter/skills) and [dart-lang/skills](https://github.com/dart-lang/skills)).

### Flutter Skills
| Skill | 用途 |
|---|---|
| `flutter-apply-architecture-best-practices` | 分层架构（UI/Logic/Data）、MVVM 模式、项目结构 |
| `flutter-setup-declarative-routing` | go_router 配置、Deep Link、嵌套路由、StatefulShellRoute |
| `flutter-build-responsive-layout` | 响应式布局、LayoutBuilder、断点适配 |
| `flutter-add-widget-test` | Widget 测试、WidgetTester 交互验证 |
| `flutter-add-widget-preview` | Widget 预览（previews.dart） |
| `flutter-add-integration-test` | 集成测试、Flutter Driver |
| `flutter-fix-layout-issues` | 布局溢出修复、约束问题诊断 |
| `flutter-implement-json-serialization` | JSON 序列化（fromJson/toJson） |
| `flutter-setup-localization` | 国际化（l10n、intl） |
| `flutter-use-http-package` | HTTP 请求（GET/POST/PUT/DELETE） |

### Dart Skills
| Skill | 用途 |
|---|---|
| `dart-add-unit-test` | 单元测试编写 |
| `dart-collect-coverage` | 测试覆盖率收集 |
| `dart-fix-runtime-errors` | 运行时错误修复 |
| `dart-generate-test-mocks` | Mock 生成（mocktail） |
| `dart-run-static-analysis` | 静态分析（dart analyze） |
| `dart-resolve-package-conflicts` | 依赖冲突解决 |
| `dart-use-pattern-matching` | Dart 3 pattern matching |
| `dart-migrate-to-checks-package` | 迁移到 checks 断言包 |
| `dart-build-cli-app` | CLI 应用构建 |
<!-- GSD:skills-end -->
